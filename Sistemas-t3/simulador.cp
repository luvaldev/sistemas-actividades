...
// definicion de colores para la consola (rubrica item 14)
...
vector<MemoryFrame> swap;
vector<Process> processes;

int ram_size_mb, page_size_kb, swap_size_mb;
int num_frames_ram, num_frames_swap;
int min_proc_size, max_proc_size; // rango en kb
int fifo_pointer = 0;             // puntero para reemplazo fifo

// inicializa memoria
void init_memory() {
    ram.resize(num_frames_ram);
    swap.resize(num_frames_swap);
    // por defecto el constructor de MemoryFrame pone occupied = false
}

// busca frame libre en ram
int find_free_ram() {
    for (int i = 0; i < num_frames_ram; i++) {
        if (!ram[i].occupied) return i;
    }
    return -1;
}

// busca frame libre en swap
int find_free_swap() {
    for (int i = 0; i < num_frames_swap; i++) {
        if (!swap[i].occupied) return i;
    }
    return -1;
}

// crea un proceso nuevo (item 2 rubrica)
void create_process(int id) {
    // tamano aleatorio dentro del rango
    int size_kb = min_proc_size + rand() % (max_proc_size - min_proc_size + 1);
    
    // calculo de paginas necesarias (division techo)
    ...
    for (int i = 0; i < pages_needed; i++) {
        ...
        if (ram_idx != -1) {
            ...
            if (swap_idx != -1) {
                swap[swap_idx].occupied = true;
                swap[swap_idx].pid = id;
                swap[swap_idx].page_num = i;
                cout << COLOR_YELLOW "  -> pagina " << i << " (proc " << id 
                     << ") asignada a swap (ram llena)" << COLOR_RESET << endl;
            } else {
                // memoria agotada (item 5 rubrica)
...
// finaliza un proceso aleatorio (item 4 rubrica)
void kill_process_random() {
    // filtrar procesos activos
    vector<int> active_indices;
    for (size_t i = 0; i < processes.size(); i++) {
        if (processes[i].active) active_indices.push_back(i);
    }
...
// simula acceso a memoria y page faults (item 3 rubrica)
void access_random_address() {
    ...
    if (!in_ram) {
        ...
        // buscar en swap
        for (int i = 0; i < num_frames_swap; i++) {
            if (swap[i].occupied && swap[i].pid == pid && swap[i].page_num == target_page) {
                swap_loc = i;
                break;
            }
        }
        ...
        if (swap_loc != -1) {
            ...
            ram[victim_frame] = swap[swap_loc];
            ram[victim_frame].occupied = true; // asegurar
            
            swap[swap_loc] = temp;
            
        } else {
            // si no esta en ram ni swap, es un error logico o el proceso murio justo antes
...
void print_status() {
    int ram_used = 0;
    for(const auto &f : ram) if(f.occupied) ram_used++;
    
    int swap_used = 0;
    for(const auto &f : swap) if(f.occupied) swap_used++;
...
int main() {
    ...
    // item 1: ingreso de parametros
    cout << "ingrese memoria fisica (mb): ";
    if (!(cin >> ram_size_mb)) return 1;

    cout << "ingrese tamano pagina (kb): ";
    if (!(cin >> page_size_kb)) return 1;

    cout << "ingrese rango procesos (min_kb max_kb): ";
    if (!(cin >> min_proc_size >> max_proc_size)) return 1;
