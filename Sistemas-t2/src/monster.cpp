  ...
  while (vivo)
  {
    ...
    if (!alertado)
    {
      for (const auto &hero : sim.heroes)
      {
        if (hero.vivo)
        {
          ...
          {
            std::cout << "monstruo " << id << " ha visto al heroe " << hero.id << std::endl;
            alertado = true;

            // alertar a otros monstruos
            for (auto &otro_monstruo : sim.monsters)
            {
              if (otro_monstruo.id != id && !otro_monstruo.alertado)
              {
            ...
            break; // ya vio a un heroe, no necesita seguir buscando
          }
        }
      }
    }
    ...
    if (!alertado)
    {
      ...
      for (const auto &hero : sim.heroes)
      {
        ...
        {
          int d = distancia_manhattan(pos, hero->pos);
          if (d <= vision_range && d < dist_min)
          {
            dist_min = d;
